...
int Process2::camProcess()
{
	...
	if(P_para->Frame_num2==1/*表示第一次检测到人脸*/)
	{
		a[0]=0;
	...
	if((a[0]==0)&&(a[1]==1))
	{
		...
		int flag_estimate=0;
		flag_estimate=align_pic(faces,P_para->regions2,P_para->Align_iterations,num+1);//对齐人脸
...
int Process2::face_detectAnddraw()
{
	...
	// cvPoint 为起笔的x，y坐标   
	cvPutText(fram_copy,showMsg,cvPoint(P_para->area2.x,P_para->area2.y-10),&font,CV_RGB(255,100,255));//在图片中输出字符 
	...
	if (faces->total==1)
	{
		for( i = 0; i < (faces ? faces->total : 0); i++ )
		{
			...
			p2.y=center.y+radius+P_para->area2.y+cvRound(2*radius*0.1);//修正值cvRound(2*radius*0.1)
		...
		if ((num>0)&&(num<6))//预备阶段
		{
			...
			cvPutText(fram_copy,showMsg,cvPoint(P_para->area2.x+P_para->regions2.x+P_para->regions2.width,P_para->area2.y+P_para->regions2.y),&font,CV_RGB(0,255,255));//在图片中输出字符 
		}

		if ((num>=6)&&(num<34))//采集的10张照片
		{
			char  showMsg[]="Begin..."; 
			cvPutText(fram_copy,showMsg,cvPoint(P_para->area2.x+P_para->regions2.x+P_para->regions2.width,P_para->area2.y+P_para->regions2.y),&font,CV_RGB(0,255,0));//在图片中输出字符 
		}
		if(num>34)//采集满35张照片,结束
		{
			char showMsg[]="OK...";
			cvPutText(fram_copy,showMsg,cvPoint(P_para->area2.x+P_para->regions2.x+P_para->regions2.width,P_para->area2.y+P_para->regions2.y),&font,CV_RGB(255,0,0));//在图片中输出字符 
		}
